...
function study(course_id,user_id){
    ...
    if(type == '开始学习'){
        //防止用户修改元素跳过加入购物车或支付步骤而开始直接学习
        $.ajax({
            cache:false,
            url:"/order/check_course/",
            type:"post",
            data:{'user_id':user_id,'course_id':course_id},
            async: true,
            beforeSend:function(xhr, settings){
            ...
            success:function(data){
                if(data.status == 'success' && data.msg != '2'){
                    window.location.href = '/course/course_info/' + course_id;
                }else{
                    alert("您还未支付");
                }
            },
        ...
        //加入购物车
        addShoppingCart(user_id,course_id);
    }else if(type == '去结算'){
        window.location.href = '/order/order_info/' + user_id;
    }else{
        alert("元素错误");
    }
}
...
$(function() {
    ...
    //     $('.shoppingcart-detail').stop(true).show();
    // },function(){
    //     $('.shoppingcart-detail').stop(true).hide();
    // });
    ...
    //绑定订单提交事件
    $("#jsOrderCommit").off().on('click',function(){
        if(confirm("确认提交订单？"))
            orderCommit();
    });
